...
import { POST } from "@/app/api/points/adjust/route";
...
describe("积分调整 API", () => {
  ...
  it("POST 正调整返回新余额", async () => {
    const res = await POST(
      new Request("http://localhost/api/points/adjust", {
        method: "POST",
    ...
    expect((await res.json()).balance.available).toBe(300);
  });

  it("POST 超额负调整返回 400", async () => {
    const res = await POST(
      new Request("http://localhost/api/points/adjust", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
